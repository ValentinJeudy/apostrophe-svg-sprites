...
module.exports = async function (apos, argv, callback) {

  var self = apos.modules['apostrophe-svg-sprites'];
  var maps = self.options.maps;
  var req = apos.tasks.getReq();

  const res = await self.find({ workflowLocale: 'en-au' }).toArray()

  console.log('res ===> ', require('util').inspect(res, { colors: true, depth: 0 }))
  ...
  function parseMap(xml, map, callback) {
    console.log('xml ===> ', require('util').inspect(xml, { colors: true, depth: 2 }))
    ...
    return xml2js.parseString(xml, function (err, result) {
      ...
      console.log('symbols ===> ', require('util').inspect(symbols, { colors: true, depth: 2 }))
  ...
  function evaluateForUpsert(svgs, callback) {
    return async.eachSeries(svgs, function (svg, callback) {
      return self.find(req, {
        ...
        console.log('docs ===> ', require('util').inspect(docs, { colors: true, depth: 2 }))
